...
<%@ page import="cn.com.brilliance.begen.model.SysUserAuthority" %>
<%@ page import="cn.com.brilliance.begen.common.Tools" %>
<%@ page import="cn.com.brilliance.begen.dao.SysAppDAO" %>
<%@ page import="cn.com.brilliance.begen.dao.SysUserAuthorityDAO" %>
<%
...
List sysUserAuthorityAppList = new ArrayList();
List sysUserAuthorityMenuList = new ArrayList();

SysUserAuthorityDAO sysUserAuthorityDAO = (SysUserAuthorityDAO) Tools.getBean("sysUserAuthorityDAO");
SysUserAuthority sysUserAuthority = new SysUserAuthority();
//sysUserAuthority.setPartyId(partyId);
sysUserAuthority.setUserId(userId);
List sysUserAuthorityList = sysUserAuthorityDAO.getSysUserAuthorityList(sysUserAuthority);
for(int k=0; k<sysUserAuthorityList.size(); k++){
	SysUserAuthority sysUserAuth = (SysUserAuthority)sysUserAuthorityList.get(k);
	String type = sysUserAuth.getAuthorityType();
	if("4".equals(type)){
		sysUserAuthorityAppList.add(sysUserAuth);
	}else if("5".equals(type)){
		sysUserAuthorityMenuList.add(sysUserAuth);		
	}
}
...
for(int i=0; i<sysAppList.size(); i++){
	...
	boolean uflag = false;
	for(int n1=0; n1<sysUserAuthorityAppList.size(); n1++){
		SysUserAuthority sua = (SysUserAuthority)sysUserAuthorityAppList.get(n1);
...
<%	
	...
	for(int n=0; n<level1List.size(); n++){
		...
		for(int a2=0; a2<sysUserAuthorityMenuList.size(); a2++){
			SysUserAuthority sua = (SysUserAuthority)sysUserAuthorityMenuList.get(a2);
...
int index2=0;
		for (int k = 0; k < level2List.size(); k++) {
			...
			if(parentMenuId2.equals(menuId1)){
			    ...
			    for(int b1=0; b1<sysUserAuthorityMenuList.size(); b1++){
				    SysUserAuthority sua = (SysUserAuthority)sysUserAuthorityMenuList.get(b1);
			...
			for (int m = 0; m < level3List.size(); m++) {
				...
				if(parentMenuId3.equals(menuId2)){
					...
					boolean uflag3 = false;
		      		for(int c1=0; c1<sysUserAuthorityMenuList.size(); c1++){
			        	SysUserAuthority sua = (SysUserAuthority)sysUserAuthorityMenuList.get(c1);
...
%>		  	  
	...
	<tr  class="mainFooter">
		<td colspan=2  class="thTitle" style="text-align:center;">
			...
			<input type="hidden" name="_type" value="_user_menu">
