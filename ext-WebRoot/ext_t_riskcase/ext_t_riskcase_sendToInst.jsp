...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{triskcaseList.view}" onclick="if(rowIndexVarOfTriskcase == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
				...
				<t:updateActionListener property="#{actionParameter.successAction}" value="ext_t_riskcase/ext_t_riskcase_form_ViewForm" />
			</h:commandLink>	
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{triskcaseList.sendToInst}" onclick="if(myCheck()){alert('请选择要下发的案例');return false}else if(confirm('确定下发')){;}else{return false;}">				   
				<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="下发" />
			</h:commandLink>							
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
...
<f:verbatim></li></f:verbatim>
			...
			<t:dataTable id="triskcaseDataTable" var="triskcase"
				...
				<h:panelGroup >                                              
					<h:selectBooleanCheckbox id="check" value="#{triskcase.checked}" />
				...
				<h:selectOneMenu id="submit2Inst" value="#{triskcase.submit2Inst}"  readonly="false"  tabindex="13"  style="width:190px;" >
                        <f:selectItems value="#{extSelectItems.belongToInstList}"/>
				</h:selectOneMenu >
				</t:column>			
