...
try{
	parent.historyIframe.addHistory('/t_rule/t_rule_form_edit_ViewForm.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{truleForm.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>','<fmt:message key="trule.t_rule_form_edit_ViewForm.heading" />(<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{truleForm.trule.id}").getValue(FacesContext.getCurrentInstance()) %>)',"1")
}catch(e){}
</script>
...
<LI class="Selected" onclick="changeChildListForm(this,'truleForm');"><a href="#"><h:outputText value="#{text['trule.t_rule_form_edit_ViewForm.heading']}" /></a></LI>
...
<f:verbatim><li></f:verbatim>
					<h:commandLink action="#{truleForm.edit}">				
						<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionview.t_rule_form_edit.2c90834916ad22cb0116b32f0233020b']}" />
						<t:updateActionListener property="#{actionParameter.successAction}" value="url:../t_rule/t_rule_form_edit_EditForm.faces?_key=#{truleForm.trule.id}"/>
					</h:commandLink>	
<f:verbatim></li></f:verbatim>
<f:verbatim><li></f:verbatim>
					<h:commandLink
						action="#{truleForm.delete}" onclick="if(!confirm('#{text['button.confirm.delete']}'))return false;">
						<f:verbatim><span class="linkDelImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionview.t_rule_form_edit.2c90834916ad22cb0116b32f0243020c']}" />
					</h:commandLink>	
<f:verbatim></li></f:verbatim>
