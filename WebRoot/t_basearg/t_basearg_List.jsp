...
try{
	parent.historyIframe.addHistory('/t_basearg/t_basearg_List.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{tbaseargList.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>',"<fmt:message key="tbasearg.t_basearg_List.heading" />","2")
}catch(e){}
</script>
	...
	<a4j:jsFunction name="loadTbaseargs" action="#{tbaseargList.initTbaseargs}" reRender="tbaseargPanel,tbaseargs" />
<script>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{tbaseargList.edit}"  onclick="if(rowIndexVarOfTbasearg == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
				<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_basearg.2c90834916ad22cb0116c1cf13460238']}" />
				<t:updateActionListener property="#{actionParameter.successAction}" value="t_basearg/t_basearg_EditForm" />
			</h:commandLink>							
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{tbaseargList.view}" onclick="if(rowIndexVarOfTbasearg == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
				<f:verbatim><span class="linkViewImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_basearg.2c90834916ad22cb0116c1cf13750239']}" />
				<t:updateActionListener property="#{actionParameter.successAction}" value="t_basearg/t_basearg_ViewForm" />
			</h:commandLink>	
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			 <h:commandLink  onclick="if(rowIndexVarOfTbasearg == -1){alert('#{text['button.selected.confirm']}')}else if(confirm('#{text['button.confirm.delete']}')){deleteTbasearg();}return false" >
				<f:verbatim><span class="linkDelImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_basearg.2c90834916ad22cb0116c1cf1385023a']}" />
			</h:commandLink>	
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
       
	<f:verbatim><li></f:verbatim>
		<h:commandLink action="#{tbaseargList.add}"  >
			<f:verbatim><span class="linkAddImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistform.t_basearg.2c90834916ad22cb0116c1cf13180237']}" />
			<t:updateActionListener property="#{actionParameter.successAction}" value="url:../t_basearg/t_basearg_EditForm.faces" />
		</h:commandLink>
	<f:verbatim></li></f:verbatim>
