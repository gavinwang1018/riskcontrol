...
try{
	parent.historyIframe.addHistory('/t_basearg/t_basearg_list_view_List.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{tbaseargList.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>',"<fmt:message key="tbasearg.t_basearg_list_view_List.heading" />","2")
}catch(e){}
</script>
	...
	<div class="mytab1">
   	    <ul class="myTabBar1">
   	      <li class="Selected"><a href="#" onmouseup="hidShowSearchImg('tbaseargQuery:searchtab')"><h:outputText value="#{text['tbasearg.tbaseargQueryForm.heading']}" /><h:graphicImage id="searchImgId" styleClass="searchImgClass" url="../images/button_showall.gif" /></a></li>
   	    </ul>
   	</div>
	
	<h:form id="tbaseargQuery" >
		<a4j:jsFunction name="queryTbaseargs" action="#{tbaseargList.query}" reRender="tbaseargPanel,tbaseargs" oncomplete="document.getElementById('tbaseargList').style.display='block';parent.dyniframesize('mainFrame')" />   
		...
		<h:panelGrid columns="2" cellspacing="0" cellpadding="0"  columnClasses="width50percent,width50percent" styleClass="width100percent"> 
             <h:panelGrid columns="2" cellspacing="2" cellpadding="5" styleClass="sectionTableSearch" columnClasses="colTitleSearch,colValueSearch">
				<h:outputLabel styleClass="fontnormal" for="type_like" value="#{text['tbasearg.type']}  #{text['query.like']}" />				
				<h:panelGroup>
...
<br>
		<div class="mytab1">
    	    <ul class="myTabBar1">
    	      <li class="Selected"><a href="#"><h:outputText value="#{text['tbasearg.t_basearg_list_view_List.heading']}" /></a></li>
    	    </ul>
    	</div>
	...
	<h:panelGrid  id="tbaseargPanel" columns="1" cellspacing="0" cellpadding="0" styleClass="mainTable" >	
		...
		<h:panelGrid columns="2" styleClass="scrollerTable" columnClasses="column1,column2">
            <t:dataScroller id="tbasearg_scroll_1" 
            ...
            <t:dataScroller id="tbasearg_scroll_2" 
                    for="tbaseargDataTable"
                    rowsCountVar="rowsCount"
                    displayedRowsCountVar="displayedRowsCountVar"
                    firstRowIndexVar="firstRowIndex"
