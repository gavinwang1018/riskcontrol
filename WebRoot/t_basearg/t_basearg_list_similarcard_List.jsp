...
try{
	parent.historyIframe.addHistory('/t_basearg/t_basearg_list_similarcard_List.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{tbaseargList.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>',"<fmt:message key="tbasearg.t_basearg_list_similarcard_List.heading" />","2")
}catch(e){}
</script>
	...
	<a4j:jsFunction name="loadTbaseargs" action="#{tbaseargList.initSimilarCardTime}" reRender="tbaseargPanel,tbaseargs" />
<script>
...
<br>
		<div class="mytab1">
    	    <ul class="myTabBar1">
    	      <li class="Selected"><a href="#"><h:outputText value="#{text['tbasearg.t_basearg_list_similarcard_List.heading']}" /></a></li>
    	    </ul>
    	</div>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{tbaseargList.edit}"  onclick="if(rowIndexVarOfTbasearg == -1){alert('#{text['button.selected.confirm']}');return false}" rendered="#{sessionScope._user_info.actionMap['action.30'] == 'true'}" >				   
				<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_basearg_list_similarcard.2c908349173e897b01173e8f26260018']}" />
				<t:updateActionListener property="#{actionParameter.successAction}" value="t_basearg/t_basearg_form_similarcard_EditForm" />
			</h:commandLink>							
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
				...
				<t:column>
					<f:facet name="header">
						<t:commandSortHeader columnName="param1" arrow="true">		                
							<h:outputText value="#{text['tbasearg.similarCard']}" />
						</t:commandSortHeader>	
				</f:facet>
