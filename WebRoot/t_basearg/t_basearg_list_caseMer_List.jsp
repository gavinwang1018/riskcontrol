...
try{
	parent.historyIframe.addHistory('/t_basearg/t_basearg_list_caseMer_List.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{tbaseargList.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>',"<fmt:message key="tbasearg.t_basearg_list_caseMer_List.heading" />","2")
}catch(e){}
</script>
	...
	<a4j:jsFunction name="loadTbaseargs" action="#{tbaseargList.initCaseMer}" reRender="tbaseargPanel,tbaseargs" />
<script>
...
<br>
		<div class="mytab1">
    	    <ul class="myTabBar1">
    	      <li class="Selected"><a href="#"><h:outputText value="#{text['tbasearg.t_basearg_list_caseMer_List.heading']}" /></a></li>
    	    </ul>
    	</div>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{tbaseargList.edit}"  onclick="if(rowIndexVarOfTbasearg == -1){alert('#{text['button.selected.confirm']}');return false}" rendered="#{sessionScope._user_info.actionMap['action.33'] == 'true'}" >				   
				<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['button.edit']}" />
				<t:updateActionListener property="#{actionParameter.successAction}" value="t_basearg/t_basearg_form_caseMer_EditForm" />
			</h:commandLink>							
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
			...
			<t:dataTable id="tbaseargDataTable" var="tbasearg"
				...
				<t:column>
					<f:facet name="header">
						<t:commandSortHeader columnName="param1" arrow="true">		                
							<h:outputText value="#{text['tbasearg.days']}" />
						</t:commandSortHeader>	
				</f:facet>
				...
				<t:column>
					<f:facet name="header">
						<t:commandSortHeader columnName="param1" arrow="true">		                
							<h:outputText value="#{text['tbasearg.times']}" />
						</t:commandSortHeader>	
				</f:facet>
				...
				<t:column>
					<f:facet name="header">
						<t:commandSortHeader columnName="param1" arrow="true">		                
							<h:outputText value="#{text['tbasearg.level']}" />
						</t:commandSortHeader>	
				</f:facet>
				<h:outputText id="param3" value="#{tbasearg.param3}" />
				</t:column>								
			</t:dataTable>
