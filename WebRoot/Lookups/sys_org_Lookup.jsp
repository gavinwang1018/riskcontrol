...
<div class="title" title="<fmt:message key="sysOrg.sysOrgList.note" />">
	<table  cellspacing=0 cellpadding=0>
        <tr>
			...
			<td>
			   <p class="titleText" ><fmt:message key="sysOrg.sysOrgList.heading" /></p>	
			</td>
   		</tr>
   </table>
</div>
...
<f:view>
	...
	<h:form id="sysOrgList">
		<h:panelGrid  columns="1" cellspacing="0" cellpadding="0" styleClass="mainTable" headerClass="mainHeader" footerClass="mainFooter">
			<t:dataTable cellspacing="2" cellpadding="0" id="sysOrgDataTable" var="sysOrg"
				rowIndexVar="rowIndex"
				value="#{sysOrgList.sysOrgList}"  rowClasses="rowsLookClass2,rowsLookClass2"
				binding="#{sysOrgList.dataTable}" styleClass="detail" style="border-top:2px solid #7ABEFF">
				<h:column>

					<h:selectBooleanCheckbox id = "select_checkbox" value="" 
						rendered="#{lookupParameter.multiSelect eq 'true'}" />
					<h:inputHidden id="id" value="#{sysOrg.orgId}" />
					<h:inputHidden id="orgId" value="#{sysOrg.orgId}" />
					<h:inputHidden id="orgName" value="#{sysOrg.orgName}" />
					<h:inputHidden id="parentOrg" value="#{sysOrg.parentOrg}" />
					<h:inputHidden id="masterOrg" value="#{sysOrg.masterOrg}" />
					<h:inputHidden id="partyId" value="#{sysOrg.partyId}" />
				</h:column>
				
                        		<t:column style="text-align:left;padding-left:3px;" headerstyleClass="headTable">
            					<f:facet name="header">
						<h:outputText value="#{text['sysOrg.orgId']}" />
					</f:facet>
				<h:outputLink onclick="returnLookupValue('#{sysOrg.orgId}','#{sysOrg.orgName}')">
						<h:outputText value="#{sysOrg.orgId}"  styleClass="linkLookUp" />
				</h:outputLink>		
				</t:column>
				
                        		<t:column style="text-align:left;padding-left:3px;" headerstyleClass="headTable">
            					<f:facet name="header">
						<h:outputText value="#{text['sysOrg.orgName']}" />
					</f:facet>
				<h:outputLink onclick="returnLookupValue('#{sysOrg.orgId}','#{sysOrg.orgName}')">
						<h:outputText value="#{sysOrg.orgName}"  styleClass="linkLookUp" />
				</h:outputLink>		
				</t:column>
				
                        		<t:column style="text-align:left;padding-left:3px;" headerstyleClass="headTable">
            					<f:facet name="header">
						<h:outputText value="#{text['sysOrg.parentOrg']}" />
					</f:facet>
				<be:outputText id="parentOrg1" value="#{sysOrg.parentOrg}">
                   <f:selectItems value="#{extSelectItems.sysOrgList}"/>
				</be:outputText>
				</t:column>
				
                        		<t:column style="text-align:left;padding-left:3px;" headerstyleClass="headTable">
            					<f:facet name="header">
						<h:outputText value="#{text['sysOrg.masterOrg']}" />
					</f:facet>
				<be:outputText id="masterOrg1" value="#{sysOrg.masterOrg}">
                   <f:selectItems value="#{extSelectItems.sysOrgList}"/>
				</be:outputText>
				</t:column>
			</t:dataTable>
	...
	<script type="text/javascript" defer>
		try{
			highlightTableRows("sysOrgList:sysOrgDataTable");
