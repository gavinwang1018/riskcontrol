...
<f:view>
	...
	<h:form id="tpermissionList">
       <a4j:jsFunction name="loadTpermissions" action="#{tpermissionList.initTpermissions}" reRender="tpermissionList,tpermissions" oncomplete="checkLookupS()" />		
		...
		<h:panelGrid columns="1" cellspacing="1" cellpadding="2" styleClass="titleTable" columnClasses="titleTdClass" >
			...
			<t:dataTable id="tpermissionDataTable" var="tpermission"
				value="#{tpermissionList.tpermissions}" rowClasses="rowsClass,rowsClass2"
				rows="50"
				sortColumn="#{tpermissionList.sortColumn}"
				sortAscending="#{tpermissionList.ascending}"
				preserveDataModel="true"
				preserveSort="true"
				binding="#{tpermissionList.dataTable}" headerClass="listTableHead" styleClass="listTableDetail" columnClasses="listTableTd"
			>				
				<t:column>

					<h:selectBooleanCheckbox id = "select_checkbox" value="" 
						rendered="#{lookupParameter.multiSelect eq 'true'}" />
					<h:inputHidden id="id" value="#{tpermission.id}" />
					<h:inputHidden id="priviledeId" value="#{tpermission.priviledeId}" />
					<h:inputHidden id="priviledeType" value="#{tpermission.priviledeType}" />
					<h:inputHidden id="roleId" value="#{tpermission.roleId}" />
				</t:column>
				<t:column>
					<f:facet name="header">
						<h:outputText value="#{text['tpermission.roleId']}" />
					</f:facet>
				<h:outputLink onclick="returnLookupValue('#{tpermission.id}','#{tpermission.id}')">					
						<h:outputText value="#{tpermission.roleId}"  styleClass="linkLookUp" />
				</h:outputLink>		
				</t:column>
			</t:dataTable>
