...
try{
	parent.historyIframe.addHistory('/t_rule_instance_group_segment/t_rule_instance_group_segment_ViewForm.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{truleInstanceGroupSegmentForm.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>','<fmt:message key="truleInstanceGroupSegment.t_rule_instance_group_segment_ViewForm.heading" />(<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{truleInstanceGroupSegmentForm.truleInstanceGroupSegment.id}").getValue(FacesContext.getCurrentInstance()) %>)',"1")
}catch(e){}
</script>
...
<LI class="Selected" onclick="changeChildListForm(this,'truleInstanceGroupSegmentForm');"><a href="#"><h:outputText value="#{text['truleInstanceGroupSegment.t_rule_instance_group_segment_ViewForm.heading']}" /></a></LI>
...
<f:verbatim><li></f:verbatim>
					<h:commandLink action="#{truleInstanceGroupSegmentForm.edit}">				
						<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionview.t_rule_instance_group_segment.2c908349172dd02b01172e4b78d8000d']}" />
						<t:updateActionListener property="#{actionParameter.successAction}" value="url:../t_rule_instance_group_segment/t_rule_instance_group_segment_EditForm.faces?_key=#{truleInstanceGroupSegmentForm.truleInstanceGroupSegment.id}"/>
					</h:commandLink>	
<f:verbatim></li></f:verbatim>
<f:verbatim><li></f:verbatim>
					<h:commandLink
						action="#{truleInstanceGroupSegmentForm.delete}" onclick="if(!confirm('#{text['button.confirm.delete']}'))return false;">
						<f:verbatim><span class="linkDelImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionview.t_rule_instance_group_segment.2c908349172dd02b01172e4b7907000e']}" />
					</h:commandLink>	
<f:verbatim></li></f:verbatim>
