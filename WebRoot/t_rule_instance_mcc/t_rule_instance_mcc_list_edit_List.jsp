...
try{
	parent.historyIframe.addHistory('/t_rule_instance_mcc/t_rule_instance_mcc_list_edit_List.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{truleInstanceMccList.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>',"<fmt:message key="truleInstanceMcc.t_rule_instance_mcc_list_edit_List.heading" />","2")
}catch(e){}
</script>
...
<br>
		<div class="mytab1">
    	    <ul class="myTabBar1">
    	      <li class="Selected"><a href="#"><h:outputText value="#{text['truleInstanceMcc.t_rule_instance_mcc_list_edit_List.heading']}" /></a></li>
    	    </ul>
    	</div>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{truleInstanceMccList.edit}"  onclick="if(rowIndexVarOfTruleInstanceMcc == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
				<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_rule_instance_mcc_list_edit.2c90834916ad22cb0116b2a70e7901ae']}" />
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{truleInstanceMccList.view}" onclick="if(rowIndexVarOfTruleInstanceMcc == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
				<f:verbatim><span class="linkViewImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_rule_instance_mcc_list_edit.2c90834916ad22cb0116b2a70e9801af']}" />
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			 <h:commandLink  onclick="if(rowIndexVarOfTruleInstanceMcc == -1){alert('#{text['button.selected.confirm']}')}else if(confirm('#{text['button.confirm.delete']}')){deleteTruleInstanceMcc();}return false" >
				<f:verbatim><span class="linkDelImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_rule_instance_mcc_list_edit.2c90834916ad22cb0116b2a70eb701b0']}" />
			</h:commandLink>	
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
	...
	<f:verbatim><li></f:verbatim>
		<h:commandLink action="#{truleInstanceMccList.add}"  >
			<f:verbatim><span class="linkAddImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistform.t_rule_instance_mcc_list_edit.2c90834916ad22cb0116b2a70e5901ad']}" />
