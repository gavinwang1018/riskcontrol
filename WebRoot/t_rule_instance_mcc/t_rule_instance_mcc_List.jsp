...
try{
	parent.historyIframe.addHistory('/t_rule_instance_mcc/t_rule_instance_mcc_List.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{truleInstanceMccList.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>',"<fmt:message key="truleInstanceMcc.t_rule_instance_mcc_List.heading" />","2")
}catch(e){}
</script>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{truleInstanceMccList.edit}"  onclick="if(rowIndexVarOfTruleInstanceMcc == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
				<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_rule_instance_mcc.2c90834916ad22cb0116b2a39e29016a']}" />
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{truleInstanceMccList.view}" onclick="if(rowIndexVarOfTruleInstanceMcc == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
				<f:verbatim><span class="linkViewImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_rule_instance_mcc.2c90834916ad22cb0116b2a39e39016b']}" />
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			 <h:commandLink  onclick="if(rowIndexVarOfTruleInstanceMcc == -1){alert('#{text['button.selected.confirm']}')}else if(confirm('#{text['button.confirm.delete']}')){deleteTruleInstanceMcc();}return false" >
				<f:verbatim><span class="linkDelImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_rule_instance_mcc.2c90834916ad22cb0116b2a39e58016c']}" />
			</h:commandLink>	
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
	...
	<f:verbatim><li></f:verbatim>
		<h:commandLink action="#{truleInstanceMccList.add}"  >
			<f:verbatim><span class="linkAddImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistform.t_rule_instance_mcc.2c90834916ad22cb0116b2a39e0a0169']}" />
