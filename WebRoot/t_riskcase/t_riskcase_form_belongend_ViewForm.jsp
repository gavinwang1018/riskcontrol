...
try{
	parent.historyIframe.addHistory('/t_riskcase/t_riskcase_form_belongend_ViewForm.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{triskcaseForm.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>','<fmt:message key="triskcase.t_riskcase_form_belongend_ViewForm.heading" />(<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{triskcaseForm.triskcase.id}").getValue(FacesContext.getCurrentInstance()) %>)',"1")
}catch(e){}

function checkContext() {
	var backContext = document.getElementById("triskcaseForm:comments8");
...
<LI class="Selected" ><a href="#"><h:outputText value="#{text['triskcase.t_riskcase_form_belongend_ViewForm.heading']}" /></a></LI>
</ul>
</div>
...
<f:verbatim><li></f:verbatim>
		            <h:commandLink action="#{triskcaseForm.back}" >
						...
						<t:updateActionListener property="#{actionParameter.successAction}" value="t_riskcase/t_riskcase_form_belongover_List" />
					</h:commandLink>
<f:verbatim></li></f:verbatim>

<f:verbatim><li></f:verbatim>
		            <h:commandLink action="#{extTriskcaseForm.backToBelong}" onclick="if(checkContext()){return false;}else if(confirm('确定退回')){;}else{return false;}">
						<f:verbatim><span class="linkDelImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="退回" />
						<t:updateActionListener property="#{actionParameter.successAction}" value="url:../t_riskcase/t_riskcase_form_belongover_List.faces?status=5" />
					</h:commandLink>
<f:verbatim></li></f:verbatim>

<f:verbatim><li></f:verbatim>
		            <h:commandLink action="#{extTriskcaseForm.closeCase}" onclick="if(confirm('确定结案')){;}else{return false;}">
						<f:verbatim><span class="linkSaveImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="结案" />
						<t:updateActionListener property="#{actionParameter.successAction}" value="url:../t_riskcase/t_riskcase_form_belongover_List.faces?status=5" />
					</h:commandLink>
<f:verbatim></li></f:verbatim>
<f:verbatim></ul></f:verbatim>
		...
		<h:panelGrid columns="1" cellspacing="0" cellpadding="0" styleClass="sectionTable" headerClass="sectionHeader">
			<f:facet name="header">
				<h:outputText value="#{text['section.t_riskcase_form_belongend.2c90834917143e52011715ca257c0118']}" />
			</f:facet>
...
<f:verbatim><li></f:verbatim>
				<h:commandLink action="#{triskcaseRecordList.view}" onclick="if(rowIndexVarOfTriskcaseRecord == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
					<f:verbatim><span class="linkViewImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_riskcase_record_list_belongend.2c90834917143e520117159994bf00b9']}"  />
					<t:updateActionListener property="#{actionParameter.successAction}" value="t_riskcase_record/t_riskcase_record_add_belongend_ViewForm" />
