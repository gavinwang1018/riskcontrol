...
try{
	parent.historyIframe.addHistory('/t_riskcase/t_riskcase_form_makeing_ViewForm.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{triskcaseForm.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>','<fmt:message key="triskcase.t_riskcase_form_makeing_ViewForm.heading" />(<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{triskcaseForm.triskcase.id}").getValue(FacesContext.getCurrentInstance()) %>)',"1")
}catch(e){}
</script>

<div class="mytab1">
    <ul class="myTabBar1"  id="listUl">
<LI class="Selected"><a href="#"><h:outputText value="#{text['triskcase.t_riskcase_form_makeing_ViewForm.heading']}" /></a></LI>
</ul>
</div>
...
<f:verbatim><li></f:verbatim>
		            <h:commandLink action="#{triskcaseForm.back}" >
						...
						<t:updateActionListener property="#{actionParameter.successAction}" value="t_riskcase/t_riskcase_form_makeing_List" />
					</h:commandLink>
<f:verbatim></li></f:verbatim>
...
<f:verbatim><li></f:verbatim>
		            <h:commandLink action="#{extTriskcaseForm.submitToBelong}" onclick="if(confirm('确定提交')){;}else{return false;}">
						<f:verbatim><span class="linkSaveImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="提交" />
						<t:updateActionListener property="#{actionParameter.successAction}" value="url:../t_riskcase/t_riskcase_form_makeing_List.faces?status=2" />
					</h:commandLink>
<f:verbatim></li></f:verbatim> 

<f:verbatim><li></f:verbatim>
		            <h:commandLink action="#{extTriskcaseForm.backToBelongByWrongSubmit}" onclick="if(confirm('确定退回')){;}else{return false;}">
						<f:verbatim><span class="linkSaveImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="退回上级机构重新派发" />
						<t:updateActionListener property="#{actionParameter.successAction}" value="url:../t_riskcase/t_riskcase_form_makeing_List.faces?status=2" />
					</h:commandLink>
<f:verbatim></li></f:verbatim> 
	...
	<h:panelGrid columns="1" cellspacing="0" cellpadding="0" styleClass="titleTable" columnClasses="titleTdClass" >
		...
		<f:verbatim><li></f:verbatim>
					<h:commandLink action="#{triskcaseRecordList.add}" >
						<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="登记结果" />
		            <t:updateActionListener property="#{actionParameter.successAction}" value="t_riskcase_record/t_riskcase_record_add_EditForm"  />
		            <t:updateActionListener property="#{triskcaseRecordList.riskcaseQueryFields}" value="#{triskcaseForm.queryFields}" />
		            <t:updateActionListener property="#{triskcaseRecordList.caseno}" value="#{triskcaseForm.triskcase.caseno}" />
		            <t:updateActionListener property="#{triskcaseRecordList.riskcaseId}" value="#{triskcaseForm.triskcasePrimaryKey}" />
					</h:commandLink>
		<f:verbatim></li></f:verbatim>
           <f:verbatim></ul></f:verbatim>
		</h:panelGroup>
