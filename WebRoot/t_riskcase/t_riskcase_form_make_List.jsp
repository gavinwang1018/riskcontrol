...
try{
	parent.historyIframe.addHistory('/t_riskcase/t_riskcase_form_make_List.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{triskcaseList.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>',"<fmt:message key="triskcase.t_riskcase_form_make_List.heading" />","2")
}catch(e){}
</script>
...
<br>
		<div class="mytab1">
    	    <ul class="myTabBar1">
    	      <li class="Selected"><a href="#"><h:outputText value="#{text['triskcase.t_riskcase_form_make_List.heading']}" /></a></li>
    	    </ul>
    	</div>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{triskcaseList.edit}"  onclick="if(rowIndexVarOfTriskcase == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
				<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_riskcase_form_make.2c90834916d142fa0116d818add805a5']}" />
				<t:updateActionListener property="#{actionParameter.successAction}" value="t_riskcase/t_riskcase_form_make_EditForm" />
			</h:commandLink>							
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
			...
			<t:dataTable id="triskcaseDataTable" var="triskcase"
				...
				<be:outputText id="urgentCnt" value="#{triskcase.urgentCnt}">
