...
try{
	parent.historyIframe.addHistory('/t_riskcase/t_riskcase_form_make_ViewForm.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{triskcaseForm.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>','<fmt:message key="triskcase.t_riskcase_form_make_ViewForm.heading" />(<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{triskcaseForm.triskcase.id}").getValue(FacesContext.getCurrentInstance()) %>)',"1")
}catch(e){}
</script>
...
<LI class="Selected" onclick="changeChildListForm(this,'triskcaseForm');"><a href="#"><h:outputText value="#{text['triskcase.t_riskcase_form_make_ViewForm.heading']}" /></a></LI>
