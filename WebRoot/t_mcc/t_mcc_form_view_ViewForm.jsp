...
try{
	parent.historyIframe.addHistory('/t_mcc/t_mcc_form_view_ViewForm.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{tmccForm.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>','<fmt:message key="tmcc.t_mcc_form_view_ViewForm.heading" />(<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{tmccForm.tmcc.id}").getValue(FacesContext.getCurrentInstance()) %>)',"1")
}catch(e){}
</script>
...
<LI class="Selected" onclick="changeChildListForm(this,'tmccForm');"><a href="#"><h:outputText value="#{text['tmcc.t_mcc_form_view_ViewForm.heading']}" /></a></LI>
...
<f:verbatim><li></f:verbatim>
					<h:commandLink action="#{tmccForm.edit}">				
						<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionview.t_mcc_form_view.2c908349169f7cda0116a417660404a5']}" />
						<t:updateActionListener property="#{actionParameter.successAction}" value="url:../t_mcc/t_mcc_form_view_EditForm.faces?_key=#{tmccForm.tmcc.id}"/>
					</h:commandLink>	
<f:verbatim></li></f:verbatim>
<f:verbatim><li></f:verbatim>
					<h:commandLink
						action="#{tmccForm.delete}" onclick="if(!confirm('#{text['button.confirm.delete']}'))return false;">
						<f:verbatim><span class="linkDelImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionview.t_mcc_form_view.2c908349169f7cda0116a417662304a6']}" />
					</h:commandLink>	
<f:verbatim></li></f:verbatim>
<f:verbatim></ul></f:verbatim>
		...
		<h:panelGrid columns="1" cellspacing="0" cellpadding="0" styleClass="sectionTable" headerClass="sectionHeader">
			<f:facet name="header">
				<h:outputText value="#{text['section.t_mcc_form_view.2c908349169f7cda0116a4176558049d']}" />
			</f:facet>
