...
try{
	parent.historyIframe.addHistory('/t_mcc/t_mcc_List.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{tmccList.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>',"<fmt:message key="tmcc.t_mcc_List.heading" />","2")
}catch(e){}
</script>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{tmccList.edit}"  onclick="if(rowIndexVarOfTmcc == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
				<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_mcc.2c908349169f7cda0116a99c04c107fb']}" />
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{tmccList.view}" onclick="if(rowIndexVarOfTmcc == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
				<f:verbatim><span class="linkViewImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_mcc.2c908349169f7cda0116a99c04d007fc']}" />
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			 <h:commandLink  onclick="if(rowIndexVarOfTmcc == -1){alert('#{text['button.selected.confirm']}')}else if(confirm('#{text['button.confirm.delete']}')){deleteTmcc();}return false" >
				<f:verbatim><span class="linkDelImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_mcc.2c908349169f7cda0116a99c04ef07fd']}" />
			</h:commandLink>	
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
	...
	<f:verbatim><li></f:verbatim>
		<h:commandLink action="#{tmccList.add}"  >
			<f:verbatim><span class="linkAddImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistform.t_mcc.2c908349169f7cda0116a99c04a107fa']}" />
