...
try{
	parent.historyIframe.addHistory('/t_riskcase_record/t_riskcase_record_list_belongend_List.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{triskcaseRecordList.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>',"<fmt:message key="triskcaseRecord.t_riskcase_record_list_belongend_List.heading" />","2")
}catch(e){}
</script>
...
<br>
		<div class="mytab1">
    	    <ul class="myTabBar1">
    	      <li class="Selected"><a href="#"><h:outputText value="#{text['triskcaseRecord.t_riskcase_record_list_belongend_List.heading']}" /></a></li>
    	    </ul>
    	</div>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{triskcaseRecordList.edit}"  onclick="if(rowIndexVarOfTriskcaseRecord == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
				<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_riskcase_record_list_belongend.2c90834917143e520117159994af00b8']}" />
				<t:updateActionListener property="#{actionParameter.successAction}" value="t_riskcase_record/t_riskcase_record_add_belongend_EditForm" />
			</h:commandLink>							
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			 <h:commandLink  onclick="if(rowIndexVarOfTriskcaseRecord == -1){alert('#{text['button.selected.confirm']}')}else if(confirm('#{text['button.confirm.delete']}')){deleteTriskcaseRecord();}return false" >
				<f:verbatim><span class="linkDelImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_riskcase_record_list_belongend.2c90834917143e520117159994ce00ba']}" />
			</h:commandLink>	
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
	...
	<f:verbatim><li></f:verbatim>
		<h:commandLink action="#{triskcaseRecordList.add}"  >
			<f:verbatim><span class="linkAddImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistform.t_riskcase_record_list_belongend.2c90834917143e5201171599949000b7']}" />
			<t:updateActionListener property="#{actionParameter.successAction}" value="url:../t_riskcase_record/t_riskcase_record_add_belongend_EditForm.faces?caseno=#{triskcaseForm.triskcase.caseno}" />
		</h:commandLink>
	<f:verbatim></li></f:verbatim>
