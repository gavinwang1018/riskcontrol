...
try{
	parent.historyIframe.addHistory('/t_rule_instance_group/t_rule_instance_group_form_ViewForm.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{truleInstanceGroupForm.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>','<fmt:message key="truleInstanceGroup.t_rule_instance_group_form_ViewForm.heading" />(<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{truleInstanceGroupForm.truleInstanceGroup.id}").getValue(FacesContext.getCurrentInstance()) %>)',"1")
}catch(e){}
</script>
...
<LI class="Selected" onclick="changeChildListForm(this,'truleInstanceGroupForm');"><a href="#"><h:outputText value="#{text['truleInstanceGroup.t_rule_instance_group_form_ViewForm.heading']}" /></a></LI>
...
<f:verbatim><li></f:verbatim>
					<h:commandLink action="#{truleInstanceGroupForm.edit}">				
						<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionview.t_rule_instance_group_form.2c90834916d142fa0116d324e897029d']}" />
						<t:updateActionListener property="#{actionParameter.successAction}" value="url:../t_rule_instance_group/t_rule_instance_group_form_EditForm.faces?_key=#{truleInstanceGroupForm.truleInstanceGroup.id}"/>
					</h:commandLink>	
<f:verbatim></li></f:verbatim>
<f:verbatim><li></f:verbatim>
					<h:commandLink
						action="#{truleInstanceGroupForm.delete}" onclick="if(!confirm('#{text['button.confirm.delete']}'))return false;">
						<f:verbatim><span class="linkDelImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionview.t_rule_instance_group_form.2c90834916d142fa0116d324e8a6029e']}" />
					</h:commandLink>	
<f:verbatim></li></f:verbatim>
