...
try{
	parent.historyIframe.addHistory('/t_riskcase_log/t_riskcase_log_list_List.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{triskcaseLogList.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>',"<fmt:message key="triskcaseLog.t_riskcase_log_list_List.heading" />","2")
}catch(e){}
</script>
...
<br>
		<div class="mytab1">
    	    <ul class="myTabBar1">
    	      <li class="Selected"><a href="#"><h:outputText value="#{text['triskcaseLog.t_riskcase_log_list_List.heading']}" /></a></li>
    	    </ul>
    	</div>
...
<f:verbatim><li></f:verbatim>
	<h:panelGroup >
			<h:commandLink action="#{triskcaseLogList.view}" onclick="if(rowIndexVarOfTriskcaseLog == -1){alert('#{text['button.selected.confirm']}');return false}" >				   
				<f:verbatim><span class="linkViewImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionlistrec.t_riskcase_log_list.2c908349169f7cda0116a4384d3c054e']}" />
				<t:updateActionListener property="#{actionParameter.successAction}" value="t_riskcase_log/t_riskcase_log_form_ViewForm" />
			</h:commandLink>	
	</h:panelGroup>
<f:verbatim></li></f:verbatim>
