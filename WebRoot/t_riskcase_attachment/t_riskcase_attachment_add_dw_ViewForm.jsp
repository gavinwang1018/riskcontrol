...
try{
	parent.historyIframe.addHistory('/t_riskcase_attachment/t_riskcase_attachment_add_dw_ViewForm.faces?<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{triskcaseAttachmentForm.historyUrlParameter}").getValue(FacesContext.getCurrentInstance())%>','<fmt:message key="triskcaseAttachment.t_riskcase_attachment_add_dw_ViewForm.heading" />(<%=FacesContext.getCurrentInstance().getApplication().createValueBinding("#{triskcaseAttachmentForm.triskcaseAttachment.id}").getValue(FacesContext.getCurrentInstance()) %>)',"1")
}catch(e){}
</script>
...
<LI class="Selected" onclick="changeChildListForm(this,'triskcaseAttachmentForm');"><a href="#"><h:outputText value="#{text['triskcaseAttachment.t_riskcase_attachment_add_dw_ViewForm.heading']}" /></a></LI>
...
<f:verbatim><li></f:verbatim>
					<h:commandLink action="#{triskcaseAttachmentForm.edit}">				
						<f:verbatim><span class="linkEditImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionview.t_riskcase_attachment_add_dw.2c908349171feac4011720229c3e0010']}" />
						<t:updateActionListener property="#{actionParameter.successAction}" value="url:../t_riskcase_attachment/t_riskcase_attachment_add_dw_EditForm.faces?_key=#{triskcaseAttachmentForm.triskcaseAttachment.id}"/>
					</h:commandLink>	
<f:verbatim></li></f:verbatim>
<f:verbatim><li></f:verbatim>
					<h:commandLink
						action="#{triskcaseAttachmentForm.delete}" onclick="if(!confirm('#{text['button.confirm.delete']}'))return false;">
						<f:verbatim><span class="linkDelImg"></span></f:verbatim><h:outputText styleClass="buttonText" value="#{text['actionview.t_riskcase_attachment_add_dw.2c908349171feac4011720229c5e0011']}" />
					</h:commandLink>	
<f:verbatim></li></f:verbatim>
<f:verbatim><li></f:verbatim>
		            <h:commandLink onclick="window.open('../t_riskcase/t_riskcase_form_view_dw_ViewForm.faces?_key=#{triskcaseAttachmentForm.triskcaseAttachment.riskcaseId}','_self','');return false" >
...
<f:verbatim></ul></f:verbatim>
		...
		<h:panelGrid columns="1" cellspacing="0" cellpadding="0" styleClass="sectionTable" headerClass="sectionHeader">
			<f:facet name="header">
				<h:outputText value="#{text['section.t_riskcase_attachment_add_dw.2c908349171feac4011720229c9c0012']}" />
			</f:facet>
